...
The Normalized Difference Vegetation Index (NDVI) \cite{Rouse} is calculated as a ration between the sum and the difference of the two Landsat bands - NIR and Red. The values of the NDVI typically vary from -1 to +1 with negative values indicating water, lower values above 0 show bare soil and land, mediocre values (around 1.30-0.50) signify sparsely vegetated areas, while higher values reaching up to +1 identify dense vegetation with healthy canopy and green foliage. The content of chlorophyll in healthy vegetation is reflected by the near-infrared (NIR) band much more compared to the other bands. At the same time, it absorbs Red light. Therefore, using these two bands in a well-established combination gives robust results indicating the presence and amount of chlorophyll in leaves. Such effectiveness of the NDVI resulted in its numerous applications for the environmental analysis \cite{w13192707,rs12244119,GHEBREZGABHER2020249,RULINDA2012169}. 
...
Developed by \cite{GAO1996257} as a complementary to the NDVI, the NDWI is strongly related to the plant water content as it is sensitive to liquid water content of leaves base don the ratio between the difference and sum of NIR and SWIR. The second version of the NDWI developed by \cite{rs5073544} uses a Green band instead of SWIR which can be used to detect surface open water areas as well as evaluate the level of turbidity. In this case, the NDWI is computed using the ratio between the difference and sum of the NIR and visible Green Bands \cite{ZHENG2021129488} in the Equation \ref{eq:2}:
...
The values of Infrared Percentage Vegetation Index (IPVI) depend the chlorophyll content in leaves. Developed by \cite{CRIPPEN199071}, it has always positive values ranging from 0 to +1 and indicates the photosynthetic activity of the canopy cover and total of chlorophylls. Further, as a modified variation it is especially suitable to indicate yellowish or shed leaves due to the adjusted spectral regions in the formula, since the chlorophyll content directly depends on nitrogen level in plants which contributes to greenness of foliage and is reflected in optical characteristics of the leaves. The IPVI is suitable for the periods of active vegetation development due to the indication of leaf pigment content and variations showing always positive values above zero, in contrast with the NDVI. The IPVI is computed using Equation \ref{eq:3}:

\begin{equation}\label{eq:3}
ReCI=\dfrac{NIR}{NIR + Red}
\end{equation}

The computation of IPVI using R library 'terra' was done by the code in Listing \ref{lst05}:
...
Figure \ref{fig04n} shows some real examples of the application of RStudio for image processing with a case of plotting IPVI index using Landsat 8 OLI/TIRS image. We find that our method works well for repetitive workflow process when several images need to be processed sequentially for several years by applying scripts modified for each scene. The minimal and maximal values were obtained from the R console as visible in the left part of the Figure \ref{fig04n}, and used as input data for Table \ref{tab02}.

\begin{figure}[H]
\centering
	\includegraphics[width=14.0 cm]{Fig_04n.jpg}
	\caption{Working process of computing the vegetation index in RStudio, R version 4.2.2. 
	\label{fig04n}}
\end{figure}
...
We provide in Table \ref{tab02} for each image triple their VI values for years 2013, 2018 and 2022, derived from the R report on data analysis, as it appears in the output console. these values were collected and summarised in Table \ref{tab02} for comparison of variations in the VI's values. Respectively, in the second to fourth columns we indicate the years of observations and output results obtained by R 'terra' library algorithm. For each image, we collected the data on minimal and maximal values to assess the general range of data. Thus, the  results of the VI values derived from the Landsat 8 OLI/TIRS images over the Khartoum region area for nine-year period  (2013, 2018 and 2022) are presented in Table \ref{tab02}. 

\begin{table}[H] 
\caption{Evaluation results of the VI values derived from the Landsat 8 OLI/TIRS images over the Khartoum region area for nine-year period  (2013, 2018 and 2022).\label{tab02}}
\newcolumntype{C}{>{\centering\arraybackslash}X}
...
\subsection{NDVI}
The images of the NDVI (Figure \ref{fig04}) allow the differentiation of vegetation areas and crop from soil in the region southwards from the Khartoum and along the flow of theGreater Nile. Negative NDVI values were classified as non-vegetation (i.e. soil and bare land), and positive NDVI values -- as vegetation. The highest NDVI values (0.80 to 1.0) identify forest areas. For the desert areas with dominating sands such as Sudan, the NDVI values do not exceed 0.50. The minimal values determined according to the histograms obtained from the NDVI images for years 2013, 2018 and 2022 were 0.27, 0.28 and 0.28 (Table \ref{tab02}), whereas, the highest determined values  were 0.72, 0.58 and 0.60 for the same years. 

\begin{figure}[H]
\makebox[0.95\linewidth][r]{%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
			\includegraphics[width=0.85\textwidth]{Fig_04a}
		\caption{}
	\end{subfigure}\hspace*{\fill}%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
		\includegraphics[width=0.85\textwidth]{Fig_04b}
		\caption{}
	\end{subfigure}%
}\\
\vfill \vspace{1mm}
\makebox[0.95\linewidth][r]{%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
			\includegraphics[width=0.85\textwidth]{Fig_04c}
		\caption{}
	\end{subfigure}%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
		\includegraphics[width=0.85\textwidth]{Fig_04d}
		\caption{}
	\end{subfigure}%
}\\
\vfill \vspace{1mm}
\makebox[0.95\linewidth][r]{%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
			\includegraphics[width=0.85\textwidth]{Fig_04e}
		\caption{}
	\end{subfigure}%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
		\includegraphics[width=0.85\textwidth]{Fig_04f}
		\caption{}
	\end{subfigure}%
}
...
\makebox[0.90\linewidth][r]{%
	...
	\begin{subfigure}[b]{.6\textwidth}
		\centering
		\includegraphics[width=0.85\textwidth]{Fig_05b}
...
\makebox[0.90\linewidth][r]{%
	...
	\begin{subfigure}[b]{.6\textwidth}
		\centering
		\includegraphics[width=0.90\textwidth]{Fig_05d}
...
\makebox[0.90\linewidth][r]{%
	...
	\begin{subfigure}[b]{.6\textwidth}
		\centering
		\includegraphics[width=0.90\textwidth]{Fig_05f}
...
\makebox[0.90\linewidth][r]{%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
			\includegraphics[width=0.85\textwidth]{Fig_06c}
		\caption{2018}
	\end{subfigure}%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
		\includegraphics[width=0.80\textwidth]{Fig_06d}
		\caption{2018: histogram}
	\end{subfigure}%
}\\
\vfill \vspace{1mm}
\makebox[0.90\linewidth][r]{%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
			\includegraphics[width=0.85\textwidth]{Fig_06e}
		\caption{2022}
	\end{subfigure}%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
		\includegraphics[width=0.80\textwidth]{Fig_06f}
		\caption{2022: histogram}
	\end{subfigure}%
}
...
\makebox[0.90\linewidth][r]{%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
			\includegraphics[width=0.85\textwidth]{Fig_07a}
		\caption{2013}
	\end{subfigure}%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
...
\makebox[0.90\linewidth][r]{%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
			\includegraphics[width=0.85\textwidth]{Fig_07c}
		\caption{2018}
	\end{subfigure}%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
		\includegraphics[width=0.83\textwidth]{Fig_07d}
		\caption{2018: histogram}
	\end{subfigure}%
}\\
\vfill \vspace{1mm}
\makebox[0.90\linewidth][r]{%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
			\includegraphics[width=0.85\textwidth]{Fig_07e}
		\caption{2022}
	\end{subfigure}%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
		\includegraphics[width=0.85\textwidth]{Fig_07f}
		\caption{2022: histogram}
	\end{subfigure}%
}
...
\makebox[0.90\linewidth][r]{%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
			\includegraphics[width=0.85\textwidth]{Fig_08a}
		\caption{2013}
	\end{subfigure}%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
...
\makebox[0.90\linewidth][r]{%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
			\includegraphics[width=0.85\textwidth]{Fig_08c}
		\caption{2018}
	\end{subfigure}%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
		\includegraphics[width=0.85\textwidth]{Fig_08d}
		\caption{2018: histogram}
	\end{subfigure}%
}\\
\vfill \vspace{1mm}
\makebox[0.90\linewidth][r]{%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
			\includegraphics[width=0.85\textwidth]{Fig_08e}
		\caption{2022}
	\end{subfigure}%
	\begin{subfigure}[b]{.6\textwidth}
		\centering
		\includegraphics[width=0.85\textwidth]{Fig_08f}
		\caption{2022: histogram}
	\end{subfigure}%
}
...
The majority of values of the Infrared Percentage Vegetation Index (IPVI) for the area of Sudan, Figure \ref{fig08} lies within very narrow interval of 0.25 to 0.48 which well corresponds with the dominance of sandy dune areas and bare soils in the surrounding deserts. Note that all the values of IPVI are never negative and always positive due to the formula used in computation, which distinct this index from the previous ones.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Discussion}

In this paper, we have proposed the application of R language for computation of several vegetation indices using library 'terra'. Compared to the traditional GIS approaches, the proposed method has demonstrated high efficiency and effectiveness for geospatial data processing aimed at environmental analysis. In the performed experiments on calculation of VIs over the Landsat 8 OLI/TIRS data, our study has shown that, in most cases of the indices, a general trend in decrease of VI values is notable for data on 2013, 2018 and 2022, as shown in Table \ref{tab02} and visualised in Figures \ref{fig04}, \ref{fig05}, \ref{fig06}, \ref{fig07} and \ref{fig08}. This is clearly visible from the results obtained using the estimated parameters on indices NDVI, OSAVI, IPVI. The proposed research technically implemented in R shows promising results on detecting anomalies in vegetation conditions in southern Sudan, as the VIs provide essential information on plant greenness reflected as chlorophyll content. The chlorophyll content related to the water content in leaves is in turn controlled by regional climatic setting and hydrological processes of the Nile River and its tributaries - the White and Blue Niles. 
