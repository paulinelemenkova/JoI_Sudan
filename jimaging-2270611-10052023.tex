...
\AuthorCitation{Lemenkova, P.; %MDPI: Please check all author names carefully. % Authors' reply: yes, correct.
 Debeir, O.}
% If this is a Chicago style journal: Lastname, Firstname, Firstname Lastname, and Firstname Lastname.

% Affiliations / Addresses (Add [1] after \address if there is only one affiliation.)
\address[1]{%
Laboratory %MDPI: If the information provided presents more than one address, please separate the addresses into different affiliations. % Authors' reply: there is only one affiliation and its only one address.
 of Image Synthesis and Analysis (LISA), École polytechnique de Bruxelles (Brussels Faculty of Engineering), Université Libre de Bruxelles (ULB), Building L, Campus du Solbosch, ULB---LISA CP165/57, Avenue Franklin D. Roosevelt 50, Brussels 1050, Belgium; olivier.debeir@ulb.be %MDPI: Newly added information. Please confirm. % Authors' reply: yes, we confirm.
...
GIS was one of the earliest computer-based applications for satellite image processing developed for the empirical investigations of the environmental parameters derived from remote sensing data. Since the 1990s, researchers were able to accurately compute the vegetation indices derived from the images using GIS. Previous studies evaluated the performance of the Earth Resources Data Analysis System (ERDAS) %MDPI: Please add definition, the following highlights are the same. % Authors' reply: corrected as follows: Earth Resources Data Analysis System (ERDAS); ENvironment for Visualizing Images (ENVI); Normalized Difference Vegetation Index (NDVI) The abbreviation is also added in the Abbreviations list.
 Imagine in computing vegetation indices to assess desertification in semi-arid regions, emphasising that the Normalized Difference Vegetation Index (NDVI) classification is suitable for assessing land degradation using the NDVI pixel range~\cite{KUMAR2022100578}. Others examined the links between the NDVI and the salinity characteristics in the surface water, groundwater and~soils using the ArcGIS~\cite{RASHID2023100314}. Furthermore, Ezaidi~et~al. applied ENvironment for Visualizing Images (ENVI) GIS for computing the NDVI for a time series of the Landsat images to assess the degradation in vegetation~\cite{EZAIDI2022100800}. All these studies illustrate the traditional approaches for computing vegetation indices by~GIS.
...
%\centering %% If there is a figure in wide page, please release command \centering % Authors' reply: corrected.
	\hspace{50pt}\includegraphics[width=17.0 cm]{Fig_01.jpg}
\end{adjustwidth}
	\caption{Topographic %MDPI: The content of the picture is covered, please modify. If the covered area does not affect the readability, please confirm. % Authors' reply: yes, we confirm that the content of the map is correct. The area of the Sudan is made more bright against the neighboring countries to focus the attention on the target area. This is done on purpose using cartographic technic of transparency added to 40% on the neighbouring countries. The area of Sudan is plotted with 100% opacity. Everything is correct here.
...
\begin{subfigure}[]{.55\textwidth}
		%\centering
			\hspace{100pt}\includegraphics[width=7cm]{Fig_02a.jpg}
		\caption{}
	\end{subfigure}\hspace{3pt}
	\begin{subfigure}[]{.55\textwidth}
		%\centering
			\hspace{100pt}\includegraphics[width=7cm]{Fig_02b.jpg}
		\caption{}
	\end{subfigure}}
\end{adjustwidth}%
%}
\caption{The confluence %MDPI: Please add the explanation for non-english character in the figure. % Authors' reply: added explanation as follows: "The non-English characters in the map present the names of the topographic objects added automatically on the Google Map and Google Earth data using national official language of the country. Here the Arabic translations are added for all the settlements and objects represented on a map of Sudan on the Google Map and Google Earth data."
 of the White Nile and Blue Nile in Khartoum, visualised based on the map and aerial images from Google Earth: (\textbf{a}) Google Earth Map; and (\textbf{b}) Google Earth~Image. The non-English characters in the map present the names of the topographic objects added automatically on the Google Map and Google Earth data using national official language of the country. Here the Arabic translations are added for all the settlements and objects represented on a map of Sudan on the Google Map and Google Earth data.}\label{fig02}
\end{figure}
...
We evaluate our approach on three datasets, with the~Landsat 8 OLI/TIRS satellite images covering the region of southern Sudan on three different days, %English editor: Please ensure that the meaning has been retained. % Authors' reply: yes, the meaning has been retained.
namely 20 December 2013, 18 December 2018 and 21 December 2022. The~satellite images were selected and downloaded from the United States Geological Survey (USGS) EarthExplorer %MDPI: Please add the images source (include the link to the database) in the Data Availability Statement section (Page 21). % Authors' reply: yes, included in the Data Availability Statement section.
open repository for satellite images, aerial photographs and~cartographic products (\href{https://earthexplorer.usgs.gov/}{https://earthexplorer.usgs.gov/} (accessed on 13 February 2023), %MDPI: Please add the access date (format: Date Month Year), e.g., accessed on 1 January 2020.
 Figure~\ref{fig03}.
...
\begin{table}[H]
\scriptsize
\caption{Satellite %MDPI: Please add the explanation for superscrip ``1''. % Authors' reply: the superscript is deleted (it does not have any explanations and was added by mistake).
 images used for computing the VIs: Landsat-8 OLI/TIRS collected from the USGS.\label{tab1}}
...
All the computations were performed using a computer MacBook Air with chip Apple M1 2020, operation system MacOS Ventura version 13.2.1, which uses 8 GB of superfast unified memory. On~this machine, the~technical advantages and capabilities of R and its geo-processing packages `terra' and `raster' were evaluated in terms of processing time. We quantified several computational aspects, which demonstrated the following results. 1. Time taken to read data: less than 1 s (i.e., the data were read instantly as soon as 'enter' was pressed); 2. Time taken to process data (s): 9 seconds to process the following commands corresponding to lines 7--20 in the case of Listing \ref{lst01} R code for computing the NDVI: vi <- function(); filenames <- paste0(); landsat <- rast(); ndvi <- vi(landsat, 5, 4); options(scipen = 10,000); and colours <- brewer.rdylgn(100). Next, the~graphical plotting of the image (lines 21--22 in example of the same Listing) was perform during 2 s, and~the plotting of the histogram (lines 24--27) took 1 s. 4. The overall processing speed took seconds %MDPI: Please check if these 2 parentheses are missing the left parentheses. % Authors' reply: yes, corrected (the parentheses are unnecessary, and we deleted them).
: 12 s for processing each Landsat OLI/TIRS image. 3. Memory utilisation (Mb) demonstrated effective parameters due to the Mac with chip Apple M1 %MDPI: Please check if should be revised to ``Apple M1'' % Authors' reply: yes, corrected as suggested.
...
\dataavailability{The source Landsat 8-9 OLI/TIRS images were downloaded from the EarthExplorer United States Geological Survey (USGS) data portal. URL: https://earthexplorer.usgs.gov/} %MDPI: Please add information about where the source images were downloaded from, and please include the link to the source.

\acknowledgments{The authors thank the reviewers for reading and review of this~manuscript. %MDPI: Please ensure that all individuals included in this section have consented to the acknowledgement. %Authors' reply: yes, all individuals included in this section have consented to the acknowledgement.
}
...
\newblock {\em  Egypt. J. Remote Sens. Space Sci.} {\bf
  2017}, {\em 20},~S21--S29.
%\newblock Water Scarcity and Harvesting, % Authors' reply: yes, we confirm to delete (not necessary).
...
\newblock {\em  Egypt. J. Remote Sens. Space Sci.} {\bf
  2017}, {\em 20},~S31--S38.
%\newblock Water scarcity and harvesting, % Authors' reply: yes, we deleted (unnecessary).
...
%\newblock Larger Asian Rivers: Climate Change, River Flow, and Watershed Management,  % Authors' reply: yes, we deleted it (unnecessary).
...
\newblock %Ecohydrology of large rivers, % Authors' reply: yes, we confirm.
...
Sulieman, H.M.;  %et~al. %MDPI: Please include the first ten authors' names before using "et al." in the references. % Authors' reply: corrected -  'et al.' is deleted. There is only one author (Sulieman, H.M.); there are no other authors in this paper.
...
%\newblock The Quaternary History of the River Nile,  % Authors' reply: yes, we deleted (unnecessary information).
...
%\newblock Adaptation to Climate Change: Perspectives Across Scales, % Authors' reply: yes, we deleted (unnecessary information).
...
%\newblock Landsat 8 Science Results, % Authors' reply: yes, we deleted it (unnecessary information).
