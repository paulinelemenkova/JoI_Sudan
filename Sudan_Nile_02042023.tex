...
Using R syntax, we computed the NDVI using script presented in \hl{the Appendix, }Listing \ref{lst01}. 
...
Using R syntax, we computed the GNDVI \hl{using script presented in the Appendix, } Listing \ref{lst02}: 
...
Using R library 'terra', we computed the NDWI \hl{using script presented in the Appendix in Listing} \ref{lst03}.
...
In R language, we computed the OSAVI \hl{using script presented in the Appendix, Listing }\ref{lst04}.
...
The computation of IPVI using R library 'terra' was done by the code \hl{presented in the Appendix, Listing }\ref{lst05}.
 
\hl{The} application of RStudio for image processing \hl{included image processing, computing and} plotting \hl{the vegetation indices} index using Landsat 8 OLI/TIRS image. We find that our method works well for repetitive workflow process when several images need to be processed sequentially for several years by applying scripts modified for each scene. The minimal and maximal values were obtained from the R console\hl{ }and used as input data for Table \ref{tab02}.
