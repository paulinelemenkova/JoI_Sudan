...
We processed the satellite images using libraries of R programming language \cite{RCoreTeam} using adapted methods from the spatial packages. The extraction of VIs for analyses of vegetation health by traditional software, which is conceptually straightforward due to raster algebra, may lead nevertheless to a consequent computing time. Considering the quite advantages of programming languages regarding automation of data processing by scripts, the algorithms of R libraries used below present a faster and more effective approach to implement for image processing. In a first step, the necessary libraries 'terra', 'RColorBrewer', 'Hmisc' and 'pals' were activated and the working folder is defined. The 'terra' library is the key package which was used for raster algebra and calculation of the VIs, while the other libraries are used as auxiliary packages for graphical refinements and proper visualization of the plots. Then, we defined the function of the VI, different for each of the VIs using relevant formulae. Finally, we uploaded the necessary Landsat bands in the active folder and performed calculation of each of the VIs by R scripts, as presented below for each corresponding index. 

The map in Figure \ref{fig01} has been plotted using Generic Mapping Tools (GMT) scripting toolset Version 6.1.1. \cite{Wessel} by applying the existing cartographic workflow \cite{Lemenkova202301,Lemenkova202230,Lemenkova202228}.
...
Developed by \cite{GAO1996257}, the NDWI is strongly related to the plant water content as it is sensitive to liquid water content of leaves. Moreover, it can be used to detect open water areas and evaluate the level of turbidity as a complementary to NDVI. The NDWI is computed using the arithmetics of the NIR Band and visible Green Band combination \cite{ZHENG2021129488} in the Equation \ref{eq:2}:
...
Using R library 'terra', we computed the NDWI as follows:

\begin{lstlisting}[language=R,caption=R code for computing the NDWI (here: a case of 2018),style=mystyle,label={lst:01}]
library(terra)
library(RColorBrewer)
library(Hmisc)
library(pals)
# Normalized Difference Water Index (NDWI) = (Green-NIR) / (Green+NIR)
# Nir = Band 5, SWIR1 = Band 3.
vi <- function(img, k, i) {
  bk <- img[[k]]
  bi <- img[[i]]
  vi <- (bi - bk) / (bi + bk)
...
The computation of RECI using R library 'terra' was done by the following code: 
\begin{lstlisting}[language=R,caption=R code for computing the RECI (here: a case of 2013),style=mystyle,label={lst:01}]
...
In R language, we computed the OSAVI using the following script: 

\begin{lstlisting}[language=R,caption=R code for computing the OSAVI (here: a case of 2022),style=mystyle,label={lst:01}]
...
Using R syntax, we computed the GNDVI as follows: 

\begin{lstlisting}[language=R,caption=R code for computing the GNDVI (here: a case of 2013),style=mystyle,label={lst:01}]
...
\caption{NDWI computed from the Landsat 8 OLI/TIRS images on confluence of the White Nile and Blue Nile, Sudan for three years (always December): (\textbf{a}) 2013, (\textbf{b}) 2018, (\textbf{c}) 2022, (\textbf{d}) histogram for 2013, (\textbf{e}) histogram for 2018, (\textbf{f}) histogram for 2022.}\label{fig04}
\end{figure}
...
\caption{RECI computed from the Landsat 8 OLI/TIRS images on confluence of the White Nile and Blue Nile, Sudan for three years (always December): (\textbf{a}) 2013, (\textbf{b}) 2018, (\textbf{c}) 2022, (\textbf{d}) histogram for 2013, (\textbf{e}) histogram for 2018, (\textbf{f}) histogram for 2022.}\label{fig05}
\end{figure}
...
\caption{GNDVI computed from the Landsat 8 OLI/TIRS images on confluence of the White Nile and Blue Nile, Sudan for three years (always December): (\textbf{a}) 2013, (\textbf{b}) 2018, (\textbf{c}) 2022, (\textbf{d}) histogram for 2013, (\textbf{e}) histogram for 2018, (\textbf{f}) histogram for 2022.}\label{fig06}
\end{figure}
...
\caption{NDVI computed from the Landsat 8 OLI/TIRS images on confluence of the White Nile and Blue Nile, Sudan for three years (always December): (\textbf{a}) 2013, (\textbf{b}) 2018, (\textbf{c}) 2022, (\textbf{d}) histogram for 2013, (\textbf{e}) histogram for 2018, (\textbf{f}) histogram for 2022.}\label{fig08}
\end{figure}
